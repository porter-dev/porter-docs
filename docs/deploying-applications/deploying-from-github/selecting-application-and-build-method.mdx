...
To deploy your application, first navigate to the "Launch" tab, where you will see three types of applications to deploy: web, worker, or job. To understand the difference between these three application types, see the [Overview](doc:overview-1).
...
If you already have a Dockerfile, you should locate the Dockerfile in your repository and select it. Once you click on the Dockerfile, you will be asked if you'd like to set the Docker build context to a different directory. The Docker build context is the current working directory of your Dockerfile -- in other words, where your Dockerfile assumes your application should be built from. For example, if your application code and entrypoint exist in the root of your repository, but your Dockerfile exists under the path `./docker/Dockerfile`, you should select the root of the repository as the Docker build context.
...
> **`Procfile` Support:** If you have a `Procfile` in the root of your repository, Porter will detect this and ask which process you'd like to run for your application. Porter will not run all processes in the Procfile per application: it will only run a single process.
...
- [Web Applications](doc:web-applications)
- [Worker Applications](doc:worker-applications)
- [Jobs and Cron Jobs](doc:overview-3)
